...
type CLUB0002_U struct {
	DT_ALT    time.Time `json:"dt_alt"`
	DESCRICAO string    `json:"descricao"`
}

func CLUB0002_carga(c *gin.Context) {
...
func CLUB0002_lookup(c *gin.Context) {
	cc := Helpgin.HelperGin(c)
	tk, err := cc.GetToken(lib.GetSecret_key())
	if err != nil {
		cc.JSON(400, gin.H{"error": err.Error()})
...
func CLUB0002_lista(c *gin.Context) {
	cc := Helpgin.HelperGin(c)
	tk, err := cc.GetToken(lib.GetSecret_key())
	if err != nil {
		cc.JSON(400, gin.H{"error": err.Error()})
...
func CLUB0002_row_id(c *gin.Context) {
	cc := Helpgin.HelperGin(c)
	tk, err := cc.GetToken(lib.GetSecret_key())
	if err != nil {
		cc.JSON(400, gin.H{"error": err.Error()})
...
func CLUB0002_insert(c *gin.Context) {
	cc := Helpgin.HelperGin(c)
	tk, err := cc.GetToken(lib.GetSecret_key())
	if err != nil {
		cc.JSON(400, gin.H{"error": err.Error()})
	...
	Qry, Values := lib.GenerateInsertQuery("CLUB0002", insert)
	lib.API_exec(Qry, Values, cc)
}
func CLUB0002_update_id(c *gin.Context) {
	cc := Helpgin.HelperGin(c)
	tk, err := cc.GetToken(lib.GetSecret_key())
	if err != nil {
		cc.JSON(400, gin.H{"error": err.Error()})
...
func CLUB0002_delete_id(c *gin.Context) {
	cc := Helpgin.HelperGin(c)
	tk, err := cc.GetToken(lib.GetSecret_key())
	if err != nil {
		cc.JSON(400, gin.H{"error": err.Error()})
	...
	sql := "delete from CLUB0002 WHERE ID  = $1"
	values := []interface{}{
		cc.Param("id"),
	}
