...
type FIN0003_I struct {
 ...
 TIPO_FORMA string `json:"tipo_forma"`
 ID_EMP uuid.UUID `json:"id_emp"`
 ID_REF string `json:"id_ref"`
 SALDO_INICIAL decimal.Decimal `json:"saldo_inicial"`
 DT_SALDO_INICIAL time.Time `json:"dt_saldo_inicial"`
} 
type FIN0003_U struct {
 DT_ALT time.Time `json:"dt_alt"`
 USER_ALT uuid.UUID `json:"user_alt"`
 DESCRICAO string `json:"descricao"`
 ATIVO string `json:"ativo"`
...
func FIN0003_carga(c *gin.Context) {           
...
func FIN0003_lookup(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
...
func FIN0003_lista(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  ...
  ,B6.email as lk_user_inc
  ,B7.email as lk_user_alt
  ,B3.razao_nome as lk_id_emp
  FROM fin0003 A1 
  LEFT JOIN user00001 B2
    ON (B2.id = A1.id_db)
  LEFT JOIN user00001 B6
    ON (B6.id = A1.user_inc)
  LEFT JOIN user00001 B7
    ON (B7.id = A1.user_alt)
...
func FIN0003_row_id(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
...
Qry, Values := lib.GenerateInsertQuery("FIN0003", insert)                                
lib.API_exec(Qry, Values, cc)                                                             
}                                                                                         
func FIN0003_update_id(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
...
func FIN0003_delete_id(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  ...
  sql := "delete from FIN0003 WHERE ID  = $1"
  values := []interface{}{                                                       
    cc.Param("id"),                                                                 
  }                                                                             
