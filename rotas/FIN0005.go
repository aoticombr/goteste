package rotas                   
                                
import (                        
 Helpgin "PARTNER/internal/gin" 
 "PARTNER/lib"                  
 "fmt"                          
 "time"                         
 "github.com/gin-gonic/gin"      
 "github.com/google/uuid"         
 "github.com/shopspring/decimal" 
)                               
type FIN0005_I struct {
 ID uuid.UUID `json:"id"`
 ID_DB uuid.UUID `json:"id_db"`
 DT_INC time.Time `json:"dt_inc"`
 USER_INC uuid.UUID `json:"user_inc"`
 ID_EMP uuid.UUID `json:"id_emp"`
 ID_FORM_PGTO uuid.UUID `json:"id_form_pgto"`
 DT_COMPETENCIA time.Time `json:"dt_competencia"`
 TIPO string `json:"tipo"`
 VALOR decimal.Decimal `json:"valor"`
 DESCRICAO string `json:"descricao"`
 ID_REF string `json:"id_ref"`
} 
type FIN0005_U struct {
 ID uuid.UUID `json:"id"`
 ID_DB uuid.UUID `json:"id_db"`
 DT_INC time.Time `json:"dt_inc"`
 USER_INC uuid.UUID `json:"user_inc"`
 ID_EMP uuid.UUID `json:"id_emp"`
 ID_FORM_PGTO uuid.UUID `json:"id_form_pgto"`
 DT_COMPETENCIA time.Time `json:"dt_competencia"`
 TIPO string `json:"tipo"`
 VALOR decimal.Decimal `json:"valor"`
 DESCRICAO string `json:"descricao"`
 ID_REF string `json:"id_ref"`
} 

func FIN0005_carga(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
   sql:=`select 
   a1.id as label, 
   a1.id as value, 
   a1.id as key,
   a1.id as id
   from fin0005 a1`
  sql += " where 1=1 "                                               
  sql += cc.GetFiltros()                                        
  lib.API_carga(sql, cc)                                            
}                                                                     
func FIN0005_lookup(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
	sql:=`select 
  a1.id as id,
  a1.id as name 
  from fin0005 a1`
  sql += " where 1=1 "                                               
  sql += cc.GetFiltros()                                        
  lib.API_lookup(sql, cc)                                            
}                                                                     
func FIN0005_lista(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
	sql:=`SELECT A1.*                 
  ,B2.email as lk_id_db
  ,B5.email as lk_user_inc
  ,B7.razao_nome as lk_id_emp
  ,B8.descricao as lk_id_form_pgto
  FROM fin0005 A1 
  LEFT JOIN user00001 B2
    ON (B2.id = A1.id_db)
  LEFT JOIN user00001 B5
    ON (B5.id = A1.user_inc)
  LEFT JOIN emp00001 B7
    ON (B7.id = A1.id_emp)
  LEFT JOIN fin0003 B8
    ON (B8.id = A1.id_form_pgto)
		`
  sql += " where 1=1 "                                               
  sql += cc.GetFiltros()                                        
  lib.API_lista(sql, cc)                                            
}                                                                     
func FIN0005_row_id(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
 sql:= `SELECT A1.*                 
                      ,A2.email as lk_id_db
                      ,A3.email as lk_user_inc
                      ,A4.razao_nome as lk_id_emp
                      ,A5.descricao as lk_id_form_pgto
                      FROM fin0005 A1 
                      LEFT JOIN user00001 A2
                        ON (A2.id = A1.id_db)
                      LEFT JOIN user00001 A3
                        ON (A3.id = A1.user_inc)
                      LEFT JOIN emp00001 A4
                        ON (A4.id = A1.id_emp)
                      LEFT JOIN fin0003 A5
                        ON (A5.id = A1.id_form_pgto)
                      WHERE A1.id = `+lib.QuotedStr(cc.Param("id"))
 lib.API_lista(sql, cc)                            
}                                                                     
func FIN0005_insert(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                    
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
var insert FIN0005_I                                                                     
if err := cc.ShouldBindJSON(&insert); err != nil {                                        
	fmt.Println(err.Error())                                                                
	cc.JSON(400, gin.H{"error": err.Error()})                                               
	return                                                                                  
	}                                                                                       
Qry, Values := lib.GenerateInsertQuery("FIN0005", insert)                                
lib.API_exec(Qry, Values, cc)                                                             
}                                                                                         
func FIN0005_update_id(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
  var update FIN0005_U              
  if err := cc.ShouldBindJSON(&update); err != nil {                       
    fmt.Println(err.Error())                                               
    cc.JSON(400, gin.H{"error": err.Error()})                              
  return                                                                 
  }                                                                        
 Qry, Values := lib.GenerateUpdateQuery("CLUB0001", update, "ID = "+lib.QuotedStr(cc.Param("id")))  
 lib.API_exec(Qry, Values, cc)                                         
}                                                                     
func FIN0005_delete_id(c *gin.Context) {           
  cc := Helpgin.HelperGin(c)                                                            
  tk, err := cc.GetToken(lib.GetSecret_key())                                           
  if err != nil {                                                                       
  	cc.JSON(400, gin.H{"error": err.Error()})                                           
  	return                                                                              
  }                                                                                     
  tk.Check()                                                                            
  err = tk.ValidateEmp()                                                                
  	if err != nil {                                                                     
  		cc.JSON(400, gin.H{"error": err.Error()})                                         
  	return                                                                              
  }                                                                                     
  sql := "delete from FIN0005 WHERE ID  = $1"
  values := []interface{}{                                                       
    cc.Param("id"),                                                                 
  }                                                                             
  lib.API_exec(sql,values, cc)                                                
  cc.JSON(200, gin.H{"message": "Success"})                                   
}                                                                             
